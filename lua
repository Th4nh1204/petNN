...
-------------COLLECT ORB--------------
local toggleFunction = function(Collect)
    OrbLootBagItems = Collect
    if Collect then
...
Tab:AddToggle({
    Name = "Collect Orb/Lootbags/Items",
    Default = OrbLootBagItems,
    Callback = toggleFunction
})
-------------HATCH EGG--------------
local NumberHatchEgg
Tab:AddSlider({
	Name = "Count HatchEgg",
	Min = 1,
	Max = 99,
	Default = NumberHatchEgg,
...
Tab:AddButton({
	...
	Callback = function()
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = 
            CFrame.new(-64.7869873046875, 159.2322235107422, 6431.361328125)
  	end    
})
Tab:AddButton({
